...
'+  File Description:
'       modAPICommonDialog.bas - All API Support for Common Dialog Controls
'
'   Product Name:
...
'       (Paul Mather - Common Dialog API Calls)
...
'       26May05 - Initial product build v1.0.72
'
'   Force Declarations
Option Explicit
...
Public Function ShowColor(ByVal hwnd As Long, Optional ByVal CenterForm As Boolean = True, Optional PreviousColor As Long) As SelectedColor
    ...
    ColorDialog.rgbResult = PreviousColor
...
Public Function CheckExtension(FilePath As String, Ext As String) As String
    Dim Ext0    As String
    Dim Ext1    As String
    Dim PDot    As Long
    
    Ext1 = "." & Ext
    PDot = InStr(1, FilePath, ".")
    If PDot = 0 Then
        CheckExtension = FilePath & Ext1
    Else
        Ext0 = LCase$(Mid$(FilePath, PDot))
        If Ext0 <> Ext1 Then
            CheckExtension = Left$(FilePath, PDot - 1) & Ext1
        Else
